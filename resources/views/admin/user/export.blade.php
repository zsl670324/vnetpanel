                ...
                <div class="portlet light bordered">
                    <div class="portlet-title">
                        <div class="caption font-dark">
                            <span class="caption-subject"> 【{{$user->username}}】连接配置信息 </span>
                        </div>
                    </div>
                        ...
                        <div class="table-scrollable table-scrollable-borderless">
                            <table class="table table-hover table-light">
                                ...
                                <tbody>
                                    @if($nodeList->isEmpty())
                                        <tr>
                                            <td colspan="5" style="font-size: 20px; text-align: center;">请先<a href="/admin/nodeList" target="_self">添加节点</a></td>
                                        </tr>
                                    @else
                                        @foreach($nodeList as $k => $node)
                                            <tr>
                                                <td>{{$k + 1}}</td>
                                                <td><a href="/admin/editNode?id={{$node->id}}" target="_blank">{{$node->name}}</a></td>
                                                <td>{{$node->server}}</td>
                                                <td>{{$node->ip}}</td>
                                                <td>
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            <h4 class="modal-title">配置信息 - {{$node->name}}</h4>
                                        </div>
                            ...
                            <div class="modal fade draggable-modal" id="link_{{$node->id}}" tabindex="-1" role="basic" aria-hidden="true">
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                                            <h4 class="modal-title">Scheme - {{$node->name}}</h4>
                                        </div>
                                        <div class="modal-body">
                                            @if($node->type == 1)
                                                <textarea class="form-control" rows="6" readonly="readonly">{{$node->ssr_scheme}}</textarea>
                                            ...
                                            @elseif($node->type == 2)
                                                ...
                                                <a href="{{$node->v2_scheme}}" class="btn purple" style="display: block; width: 100%;margin-top: 10px;">打开 V2Ray</a>
                                            @else
                                                <textarea class="form-control" rows="8" readonly="readonly">{{$node->trojan_scheme}}</textarea>
                                                <a href="{{$node->trojan_scheme}}" class="btn purple" style="display: block; width: 100%;margin-top: 10px;">打开 Trojan</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="modal fade" id="qrcode_{{$node->id}}" tabindex="-1" role="dialog" aria-hidden="true">
                                <div class="modal-dialog modal-sm">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                                            <h4 class="modal-title">二维码 - {{$node->name}}</h4>
                                        </div>
                                        <div class="modal-body">
                                            <div class="row">
                                                @if($node->type == 1)
                                                    <div class="col-md-12">
                                                        ...
                                                        <div style="text-align: center;"><a id="download_qrcode_ssr_img_{{$node->id}}">下载</a></div>
                                                    </div>
                                                @elseif($node->type == 2)
                                                    <div class="col-md-12">
                                                        <div id="qrcode_v2ray_img_{{$node->id}}" style="text-align: center;"></div>
                                                        <div style="text-align: center;"><a id="download_qrcode_v2ray_img_{{$node->id}}">下载</a></div>
                                                    </div>
                                                @else
                                                    <div class="col-md-12">
                                                        <div id="qrcode_trojan_img_{{$node->id}}" style="text-align: center;"></div>
                                                        <div style="text-align: center;"><a id="download_qrcode_trojan_img_{{$node->id}}">下载</a></div>
                                                    </div>
                                                @endif
                                            </div>
                                        </div>
