        ...
        <div class="row">
            <div class="col-md-12">
                <div class="note note-info">
                    <p>{{trans('home.promote_invite_code', ['referral_percent' => \App\Components\Helpers::systemConfig()['referral_percent'] * 100])}}</p>
                </div>
            </div>
        </div>
                    ...
                    <div class="portlet light bordered">
                        ...
                        <div class="portlet-body">
                            <div class="alert alert-info">
                                ...
                                {{trans('home.invite_code_tips1')}} <strong> {{Auth::user()->invite_qty}} </strong> {{trans('home.invite_code_tips2', ['days' => \App\Components\Helpers::systemConfig()['user_invite_days']])}}
                            </div>
                            <button type="button" class="btn blue" onclick="makeInvite()" @if(!Auth::user()->invite_qty) disabled @endif> {{trans('home.invite_code_button')}} </button>
                        </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tbody>
                                        ...
                                        @else
                                            @foreach($inviteList as $key => $invite)
                                                <tr>
                                                    <td> {{$key + 1}} </td>
                                                    <td> <a class="copy" data-clipboard-text="{{\App\Components\Helpers::systemConfig()['website_url']}}/register?aff={{Auth::user()->id}}&code={{$invite->code}}">{{$invite->code}}</a> </td>
    ...
    @if(!$inviteList->isEmpty())
        <script type="text/javascript">
            ...
            clipboard.on('success', function(e) {
                layer.alert("复制成功，您可以直接粘贴发送邀请链接", {icon:1, title:'提示'});
            });
