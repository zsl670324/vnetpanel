...
if (is_file($lockFile)) {
    ...
    foreach ($checkDirs as $k => $v) {
        if (!is_dir(ROOT_PATH . $v)) {
            $errInfo = '请先在' . $sitename . "根目录下执行<b>php composer.phar install</b> 安装依赖";
            break;
        }
    }
}
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $DB_PORT = isset($_POST['mysqlHostport']) ? trim($_POST['mysqlHostport']) : 3306;
    $hostArr = explode(':', $DB_HOST);
    if (count($hostArr) > 1) {
        $DB_HOST = $hostArr[0];
        $DB_PORT = $hostArr[1];
    }
    $DB_USERNAME = isset($_POST['mysqlUsername']) ? trim($_POST['mysqlUsername']) : 'vnetpanel';
    $DB_PASSWORD = isset($_POST['mysqlPassword']) ? trim($_POST['mysqlPassword']) : 'vnetpanel';
    $DB_DATABASE = isset($_POST['mysqlDatabase']) ? trim($_POST['mysqlDatabase']) : 'vnetpanel';
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlDatabase" value="vnetpanel" required="">
                </div>

                <div class="form-field">
                    <label>MySQL 用户名</label>
                    <input type="text" name="mysqlUsername" value="vnetpanel" required="">
                </div>
            ...
            <div class="form-buttons">
                <button type="submit" <?php echo $errInfo ? 'disabled' : '' ?>>安装</button>
            </div>
        </form>
                ...
                $('form').on('submit', function (e) {
                    ...
                    $.post('', $(this).serialize())
                        .done(function (ret) {
                            if (ret === 'success') {
                                $('#error').hide();
                                $("#success").text("安装成功，请使用[用户名：admin、密码：123456]登录").show();
